...
public class EditProject extends CreateEditProjectFeature {
    ...
    public EditProject(final String projectID) {
	super(3);
	...
	sendButton.addClickHandler(new ClickHandler() {
		...
		com.editProject(projectID, name.getText(), description.getText(),
			new Integer(nInitialVotes.getText()).intValue(), Cookies.getCookie("fears"), addProjectCB);
	...
	com.getProject(projectID, getProject);
    }

    protected void updateProject(ViewProject p) {
	name.setText(p.getName());
	description.setText(p.getDescription());
	nInitialVotes.setText(new Integer(p.getInitialVotes()).toString());
	Fears.getPath().setEditProject(p.getName(), projectID);
    }

    AsyncCallback<Object> getProject = new FearsAsyncCallback<Object>() {
