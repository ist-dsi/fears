...
public class Project extends Project_Base {
	...
	while (s.hasMoreTokens()) {
	    ...
	    int i = 0;
	    while (i < ret.size()) {
		if (!ret.get(i).getDescription().toLowerCase().contains(token)
			&& !ret.get(i).getName().toLowerCase().contains(token)) {
		    ret.remove(i);
		    i--;
		}
    ...
    private List<ViewFeatureResume> sortAndPage(List<ViewFeatureResume> ret, int sort, int page) {
	...
	return new ViewProject(getName(), getIdInternal(), getDescription(), getNFeatures(), getAuthorName(), FearsServiceImpl
		.getNickName(getAuthorName()), getInitialVotes(), getListPosition());
    ...
    public void edit(String name, String description, int nvotes) {
	setName(name);
	setDescription(description);
	setInitialVotes(nvotes);
