...
public class FearsApp extends FearsApp_Base {
	...
	for (FeatureRequest f : features) {
	    ...
	    res.add(new ViewFeatureResume(f.getProject().getName(), f.getProject().getIdInternal(), f.getName(), f.getWebID(), f
		    .getState(), userHasvoted, f.getDescription(), f.getVotes(), f.getNComments(), FearsServiceImpl.getNickName(f
		    .getAuthor().getUser().getName()), new Long(f.getAuthor().getUser().getOid()).toString(), f.getCreatedTime()
		    .toString(DateTimeFormat.forPattern(DateFormat.DEFAULT_FORMAT))));
	}
