...
public class Comment extends Comment_Base {
	...
	return new ViewComment(getComment(), FearsServiceImpl.getNickName(getAuthor().getUser().getName()), new Long(getAuthor()
		.getUser().getOid()).toString(), getNewState(), getOldState(), getCreatedTime().toString(
		DateTimeFormat.forPattern(DateFormat.DEFAULT_FORMAT)));
    }
}
